...
- **内嵌 Pi（IT / 策划）**：GUI 走**常驻 Pi RPC**（`--mode rpc`）；默认 **信任原生工具（S1）**——read/bash/edit/write 在会话内直接执行。产品闸门（如 brief export）仍由 skill + 宿主保留；Hermes/Codex/Cursor 的 mid-turn 审批模型不变。
...
- **冷启动**：记住上次选中的工程（`localStorage`：`activeBrief` + `lastBrief`）。「新建项目」会暂时解绑；重启仍尽量恢复上次工程，避免文档侧栏空白。
- **新对话**：同工程开新会话，**不**解绑顶栏；只有「新建项目」才清绑定。
- **文档侧栏**：工程内文件列表约展示 1.5 个条目高度（可滚动）；下方预览区为主阅读区。列表被挤没时检查侧栏高度 / 刷新文档。
