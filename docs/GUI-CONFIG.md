...
|------|----------|
| **Pi** | 默认 Provider、模型 |
| **Hermes** | 默认 Provider；**YOLO**（默认开；关则开跑报错，未接 ACP） |
| **Codex** | 用第三方开关；**沙箱** `sandbox`（默认 `workspace-write`）；开启第三方时选 Provider + 模型 |
| **Cursor** | **权限模式** `permission_mode`（默认 `force`）；本机登录/订阅 |

安全字段写入 `agents.executors.<工具>`，下一轮 `agent turn` 组装 CLI argv。详见 Feature B v2 Spec。
...
**项目经理 / 程序员（Codex 登录态、Cursor）**：聊天顶栏从本机 CLI **实时读取**模型列表（`agent --list-models` / `codex debug models`），经 `setup executor models --json`。列表为空时不展示假选项，提示登录/安装；可用「刷新」重拉。高/中/低档仅当偏好 id 出现在实时列表中时可点。仍写入 `agents.instances.<id>.model`。Codex 勾「第三方」时仍走 Provider + 手填。
