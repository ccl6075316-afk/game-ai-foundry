...
**同事实例可覆盖**（雇人弹窗 + 对话顶栏）：可选写入 `agents.instances.<id>` 的 `sandbox` / `permission_mode` / `yolo`（仅当前执行器相关键生效）。下拉首项「继承全局」= **不写/删除**该键，继续跟 Agent 预设；换执行器后历史键可留在盘上但不串用。实例可比全局更松或更紧。Hermes 实例或全局生效 `yolo=false` 时仍拒绝开跑（未接 ACP）。
