...
**项目经理 / 程序员（Codex 登录态、Cursor）**：聊天顶栏配置条用「高 / 中 / 低」档位与模型目录下拉切换模型；选中后写入 `agents.instances.<id>.model`，每轮对话传给 CLI。实例 `model` 为空时界面默认展示该执行器的中档（`tiers.mid`），首次选档或改模型时才落盘。Codex 勾选「第三方」时仍选 Provider + 手填或目录模型。
