...
| ① 后端 | 可保持 **薄 Chat**（落实才写盘） | 带终端的万能 Agent 环 |
...
| 工种 | 用户怎么叫 | 职责 | 对话后端 |
|------|------------|------|----------|
| **① 策划 / Brief** | 主对话、出 brief | 商量设计；用户明确「落实」才写 `brief.json` | **薄 Chat**：`host-chat` → `commit-brief`；**不是** Agent 环 |
...
| 工程蓝图 | `production.json` | 施工蓝图 |
| 进度与任务 | `progress.json`、task / handoff 包 | 派工与续作真相 |
| 工程与资产 | `games/`、`pipeline/` 产出 | 实现真相 |
...
宿主：`host-chat` → 落实意图切 `commit-*` → 仅 `ready_to_export` 写盘。  
CLI：`brief chat *`；会话 `plans/conversations/brief/<id>.json`。
...
| ① `brief chat` / host-chat → 落实才写 brief | 首次启动引导流 |
