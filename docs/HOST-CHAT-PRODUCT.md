...
|--|--|
| **读者** | 产品 / GUI / 接手 Agent |
| **日期** | 2026-07-17（① 后端勘误：2026-07-20） |
| **状态** | **心智已拍板**；① 原为 `brief chat` / host-chat；**决策变更为内置 Pi 会话**（见下）；②③ `agent turn`→CLI；分诊可写 `plans/handoffs/` + progress；GUI 同事多实例已有 |
...
| 制作审查 | `makeability-critic.md` | 独立子 LLM；`intent_gaps` / `detail_gaps`；export 门闩；优先读可选 `scenes`/`systems`，短 loop/description 不误判 |
| 补全细节 | `brief-enrich.md` | 按钮触发；开放式加厚玩家可见流程/呈现/参数名 + 资产候选；可带 hint 多次；屏→可选 `project.scenes[]`，跨屏规则→`project.systems[]`，UI 块→`project.ui_panels[]`；`description` 保持短总览（不自动生成示意） |
| 生成 UI 示意 | — | 按需按钮；根据 `ui_panels` 写出工程内 `ui-wireframe.md` 字符布局；无 panels 则提示先聊 UI |
| 议题头脑风暴 | `topic-brainstorm-persona.md` | 多角色并行方案 → 先拣再 `brainstorm-apply` 写回；可选 `agents.brainstorm_models` |
...
**补全细节 / 议题头脑风暴**（不绑导出）：`brief chat enrich [--hint]`；`brief chat topic-brainstorm --topic …` → `brainstorm-apply --proposal-id …`。数值可进 production；**需要哪些参数**与呈现应在 brief 加厚中出现（`scenes` / `systems` / `ui_panels` 可选分条）。不定死通用 screens schema；**不要**把系统规则堆回 `description`。  
...
- GUI Provider / Agent 预设 / 雇人与对话配置：`docs/GUI-CONFIG.md`
- 进度：`ROADMAP.md`
